...
Coef0_simu <- function(data, nSNP, theta1=theta1, theta2=theta2, itmax=500, epsilon=0.001)
{
  # theta1=1
  # theta2=1
  # nSNP=nCG
  ...
  for(iter0 in 1:itmax)# 外循环：估计方差分量 theta1, theta2, ro
  {
    ...
    for(iter in 1:itmax)# 内循环：估计betaR,gammaR,betaS,gammaS,u,v
    {
      ...
      Info33 <- t(z) %*% R11 %*% z + diag(rep((1/(theta1)),n))#n*n ***
      Info34 <- matrix(rep(0,n*n), ncol=n) #n*n  ***
      
      Info41 <- t(Info14) #n*(nb+nSNP)
      Info42 <- t(Info24) #n*(nb+nSNP)
      Info43 <- t(Info34) #n*n  ***
      Info44 <- t(z) %*% R22 %*% z + diag(rep((1/(theta2)),n)) #n*n  ***
      ...
      First.u <- t(z) %*% First.yetaS - ((u*theta2)/(theta1*theta2))# ***
      First.v <- t(z) %*% First.ksiR - ((v*theta1)/(theta1*theta2))# ***
  ...
  if(flag.var == 1)
  {
    ...
    Info31 <- t(Info13)
    Info32 <- t(Info23)
    Info33 <- t(z) %*% R11 %*% z + diag(rep((1/(theta1)),n))
    Info34 <- matrix(rep(0,n*n), ncol=n)
    Info41 <- t(Info14)
    Info42 <- t(Info24)
    Info43 <- t(Info34)
    Info44 <- t(z) %*% R22 %*% z + diag(rep((1/(theta2)),n))
    Info1 <- cbind(Info11, Info12, Info13, Info14)
    Info2 <- cbind(Info21, Info22, Info23, Info24)
    Info3 <- cbind(Info31, Info32, Info33, Info34)
    Info4 <- cbind(Info41, Info42, Info43, Info44)
    ...
    Omega <- rbind(cbind(diag(rep(theta1,n)),diag(rep(0,n))),
                   cbind(diag(rep(0,n)),diag(rep(theta2,n))))
    
    Omega.inverse <- (rbind(cbind(diag(rep(theta2,n)),diag(rep(0,n))),
                            cbind(diag(rep(0,n)),diag(rep(theta1,n)))) / 
                        (theta1 * theta2))
    ...
    ORo22 <- 2 * theta1 * theta2 * theta2
    Omega1_theta2_12 <- diag(rep(0,n))
    Omega1_theta2_21 <- diag(rep(0,n))
    ...
    ORo33 <- theta1 * theta2 
    Omega1_ro11 <- diag(rep(0,n))
    Omega1_ro12 <- diag(rep(0,n))
    Omega1_ro21 <- diag(rep((-ORo31 * ORo32)/ORo33,n))
    Omega1_ro22 <- diag(rep(0,n))
    ...
    result <- list(Diverge=Diverge,flag.para=flag.para,flag.var=flag.var,
                   ...
                   se.theta1=se.theta1,se.theta2=se.theta2,
                   ksiR=ksiR,yetaS=yetaS,u=u,v=v)
  }else{
