...
pd.all <- read.delim("TCGA.COAD.sampleMap_COAD_clinicalMatrix",header = T)
data <- read.delim("F:/real data/TCGA-COAD/COAD_survival.txt")

pd.all <- pd.all[match(rownames(data),rownames(pd.all)),]
...
pd <- pd.all[,c("sampleID","vital_status","days_to_death","days_to_last_followup","lost_follow_up",
                "PFS_Status","PFS","primary_therapy_outcome_success","gender","height","weight",
...
pd$X2 <- pd$age_at_initial_pathologic_diagnosis
...
save(betaData, file = "F:/real data/TCGA-COAD/betaData.RData")
save(pd, file = "F:/real data/TCGA-COAD/pd.RData")
